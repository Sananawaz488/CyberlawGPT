...
# Current Groq model recommended for new applications.
...
def clean_text(text: str) -> str:
    """Normalize extracted PDF text without destroying section structure."""
...
def extract_pdf_pages(pdf_path: str):
    """Extract text page-by-page so page numbers can be shown as citations."""
...
def split_text(text: str, chunk_size=CHUNK_SIZE, overlap=CHUNK_OVERLAP):
    ...
    It is intentionally simple so the project remains easy to deploy.
    """
    ...
    while start < len(text):
        ...
        # Prefer a paragraph or sentence boundary near the target end.
        boundary_candidates = [
            text.rfind("\n\n", start, end),
...
def build_knowledge_base(pdf_path: str, pdf_signature: str):
    """
    Cached by PDF signature. If the PDF changes, Streamlit automatically
    creates a fresh knowledge base.
    """
    del pdf_signature  # Used only as part of the cache key.
...
def file_signature(path: str) -> str:
    """Create a stable signature so changed PDFs trigger new embeddings."""
...
def retrieve(query, index, chunks, top_k=5):
    ...
    query_embedding = np.asarray(query_embedding, dtype="float32")scores, indices = index.search(
        query_embedding,
...
def response_instruction(level: str, size: str, answer_style: str):
    level_instructions = {
        "Beginner": (
    ...
    size_instructions = {
        "Short": "Keep the answer concise, usually 1–3 short paragraphs.",
        "Medium": "Give a balanced answer with useful detail and bullets where appropriate.",
        "Detailed": "Give a thorough answer with structured headings, relevant sections, implications, and limitations.",
    }

    style_instructions = {
        "Simple explanation": "Prioritize clarity and practical understanding.",
        "Section-focused": "Prioritize the relevant Act section(s), wording, offence, punishment, and conditions.",
        "Case-style analysis": (
            "Analyze the user's scenario carefully: identify the potentially "
            "relevant provision, explain why it may apply, and state what "
            "facts could change the analysis."
        ),
    }
...
2. Do not invent sections, penalties, definitions, procedures, authorities,
   dates, or legal rights.
...
6. When relevant, cite the source using the PDF page number shown in the
   context, for example: "PDF page 12".
7. If the question describes a hypothetical situation, explain the potentially
   relevant provision but do not claim that a court would definitely reach
   a particular outcome.
8. This is an educational information tool, not legal representation or
   professional legal advice.
9. Do not help a user commit, conceal, evade, or facilitate cybercrime.
10. If a request asks how to perform wrongdoing, do not provide operational
    instructions. You may instead explain the relevant legal prohibition,
    risk, or lawful defensive concept.
...
with st.sidebar:
    ...
    technical_level = st.selectbox(
        ...
        index=0,
        help="Controls how technical the legal explanation should be.",
    )
    ...
    top_k = st.slider(
        ...
        max_value=8,
        value=5,
        help="Number of relevant PDF passages sent to the language model.",
    )
    ...
    uploaded_pdf = st.file_uploader(
        ...
        type=["pdf"],
        help="Optional when cyberlaw.pdf already exists in the project folder.",
    )

    if uploaded_pdf is not None:
        uploaded_path = Path("uploaded_cyberlaw.pdf")

        if (
            not uploaded_path.exists()
            or st.session_state.get("uploaded_pdf_name") != uploaded_pdf.name
        ):
            uploaded_path.write_bytes(uploaded_pdf.getbuffer())
    ...
    st.caption(
        ...
        "Always verify important legal matters with a qualified legal professional."
    )
...
st.markdown(
    '<div class="subtitle">'
    "AI-powered RAG assistant for the supplied Pakistani cyber-law document"
    "</div>",
    unsafe_allow_html=True,
)
...
if not pdf_path:
    st.warning(
        "No cyber-law PDF was found. Upload the PDF from the sidebar, "
        "or place it in the project folder as `cyberlaw.pdf`."
    )
    st.stop()
...
try:
    ...
    with st.spinner(
        ...
        index, chunks = build_knowledge_base(pdf_path, signature)

    st.success(
        f"Knowledge base ready — {len(chunks)} searchable passages indexed."
    )

except Exception as error:
    st.error(f"Knowledge-base setup failed: {error}")
    st.stop()
...
question = st.text_area(
    ...
    height=120,
    placeholder=(
        "Example: What is unauthorized access under the Act, "
        "and what punishment is provided?"
    ),
)
...
if ask:
    ...
    with st.spinner("Searching the cyber-law document..."):
        retrieved = retrieve(
            question.strip(),
            index,
            chunks,
    ...
    if not retrieved:
        st.warning("No relevant passage was found.")
        st.stop()
    ...
    for number, item in enumerate(retrieved, start=1):
        with st.expander(
            f"Source {number} — PDF page {item['page']} — similarity {item['score']:.3f}"
        ):
            st.write(item["text"])

    st.info(
        "Legal information only: this answer is generated from the supplied "
        "document and should not be treated as professional legal advice."
    )
